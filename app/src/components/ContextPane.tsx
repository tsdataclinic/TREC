...
function ContextPane({
  ...
  return (
    <div id="ContextPane" className="bg-white w-96 h-5/6 t-100 fixed z-50 left-5 rounded-md">
      ...
      <div>
        <label>Field 1: </label>
        <select
          onChange={(e) => setSelectedProperties([e.target.value, selectedProperties[1]])}
          placeholder={'Select a field...'}
          defaultValue={selectedProperties[0]}>
            <option></option>
          {Array.from(availableProperties).map(p => (
            <option
              selected={p === selectedProperties[0]}>
                {p}
            </option>)
          )}
        </select>
      ...
      <div>
        <label>Field 2: </label>
        <select
          onChange={(e) => setSelectedProperties([selectedProperties[0], e.target.value])}
          placeholder={'Select a field...'}
          defaultValue={selectedProperties[1]}>
            <option></option>
          {Array.from(availableProperties).map(p => (
            <option
              selected={p === selectedProperties[1]}>
                {p}
            </option>)
          )}
        </select>
