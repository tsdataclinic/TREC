...
export const useRemoteLayerPropertyValues = (remoteLayers: RemoteLayer[], selectedProperties: Array<string>): Record<any, any> => {
  const result = useMemo(() => {
    let result: any = {};
    remoteLayers.forEach(rl => {
      if (rl.isSuccess) {
        selectedProperties.forEach(key => {
          if (!Object.keys(rl.data.features[0].properties).includes(key)) return;
